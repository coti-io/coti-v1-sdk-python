...
coti_native_hash = 'ae2b227ab7e614b8734be1f03d1532e66bf6caf76accc02ca4da6e28'
...
def get_address_balance(address_id, full_node_address, currency_hash):
    if coti_native_hash == currency_hash:
        return get_address_native_balance(address_id, full_node_address);
    else:
        return get_address_non_native_balance(address_id, full_node_address, currency_hash)


def get_address_non_native_balance(address_id, full_node_address, currency_hash):
    start_time = time.time()

    payload = "{\n\"addresses\": [\"" + address_id + "\"]\n}\n"
    headers = {'Content-Type': "application/json"}
    res = http_pool_manager.request("POST", full_node_address + "/balance/tokens", body=payload, headers=headers)
    ...
    return data["tokenBalances"][address_id][currency_hash]["addressBalance"]


def get_address_native_balance(address_id, full_node_address):
    start_time = time.time()
...
def create_full_node_fee(full_node_address, user_hash, private_key, original_currency_hash, fee_currency_hash,
                         amount, fee_included):
    start_time = time.time()
    ...
    body = {
        ...
        "currencyHash": fee_currency_hash,
        "userHash": user_hash,
...
def build_receiver_base_transaction(instant_time, instant_time_millisecond, amount_sum, original_amount, currency_hash,
                                    fee_currency_hash,
                                    destination_address):
    start_time = time.time()

    receiver_base_transaction = {"addressHash": destination_address, "amount": amount_sum,
                                 "originalAmount": original_amount,
                                 "createTime": instant_time, "name": "RBT", "currencyHash": currency_hash,
                                 "originalCurrencyHash": fee_currency_hash}
    msg_bytes = bytearray.fromhex(receiver_base_transaction["addressHash"]) \
                ...
                + bytes.fromhex(fee_currency_hash) \
...
def create_input_base_transaction(currency_hash, full_node_fee, instant_time, instant_time_millisecond,
                                  network_fee_data, source_address, context, fee_included, ibt_only_fee):
    start_time = time.time()

    if ibt_only_fee and fee_included:
        raise ValueError('cant include fee in a only fee input base transaction')

    fee_amount = Decimal(full_node_fee['amount']) + Decimal(network_fee_data['amount']).normalize(context)

    if ibt_only_fee:
        full_amount = fee_amount
    else:
        full_amount = Decimal(full_node_fee['originalAmount'])
        if fee_included:
            full_amount = full_amount + fee_amount
...
def call_apis_to_prepare_a_tx(source_address_public_key_crc, destination_address, amount, currency_hash,
                              fee_currency_hash, seed,
                              address_private, transaction_description, full_node_address, trust_score_address,
                              fee_included, fee_address_private, fee_source_address_public_key_crc):
    start_time = time.time()
    ...
    full_node_fee = create_full_node_fee(full_node_address, public_key, private_key, currency_hash, fee_currency_hash,
                                         amount, fee_included)
    ...
    receiver_base_transaction = build_receiver_base_transaction(instant_time, instant_time_millisecond, amount, amount,
                                                                currency_hash, currency_hash, destination_address)

    input_base_transactions = []

    input_base_transaction = create_input_base_transaction(currency_hash, full_node_fee, instant_time,
                                                           instant_time_millisecond, network_fee_data,
                                                           source_address_public_key_crc, context, fee_included, False)

    input_base_transactions.append(input_base_transaction)

    if currency_hash is not fee_currency_hash:
        fee_input_base_transaction = create_input_base_transaction(fee_currency_hash, full_node_fee, instant_time,
                                                                   instant_time_millisecond, network_fee_data,
                                                                   fee_source_address_public_key_crc, context,
                                                                   fee_included, True)

        input_base_transactions.append(fee_input_base_transaction)

    transaction_hash = update_ibt_signature(address_private, fee_address_private, fee_input_base_transaction,
                                            full_node_fee, input_base_transaction, network_fee_data,
                                            receiver_base_transaction)

    transaction_trust_score_data = get_trust_score_data(trust_score_address, public_key, private_key, transaction_hash)

    api_call_times_logger.info(
        "\t\t---> call_apis_to_prepare_a_tx : %s seconds <---" % round((time.time() - start_time), 3))

    return full_node_fee, input_base_transactions, instant_time, instant_time_millisecond, network_fee_data, \
        private_key, public_key, receiver_base_transaction, transaction_description, transaction_hash, \
        transaction_trust_score_data


def update_ibt_signature(address_private, fee_address_private, fee_input_base_transaction, full_node_fee,
                         input_base_transaction, network_fee_data, receiver_base_transaction):
    if fee_input_base_transaction is None:
        transaction_hash_bytes = input_base_transaction["hash"] + full_node_fee["hash"] + network_fee_data["hash"] + \
                                 receiver_base_transaction["hash"]
    else:
        transaction_hash_bytes = input_base_transaction["hash"] + fee_input_base_transaction["hash"] + full_node_fee[
            "hash"] + network_fee_data["hash"] + receiver_base_transaction["hash"]
    ...
    input_base_transaction["signatureData"] = ibt_signature_data

    if fee_input_base_transaction is not None:
        fee_transaction_hash = HashKeccak256(bytearray.fromhex(transaction_hash_bytes))
        fee_address_signed_transaction_hash = SignDigest(bytearray.fromhex(fee_address_private),
                                                         bytearray.fromhex(fee_transaction_hash))
        fee_ibt_signature_data = {"r": fee_address_signed_transaction_hash[0], "s": fee_address_signed_transaction_hash[1]}
        fee_input_base_transaction["signatureData"] = fee_ibt_signature_data
    return transaction_hash


def create_transaction(full_node_address, full_node_fee, input_base_transactions, instant_time,
                       instant_time_millisecond,
                       network_fee_data,
                       private_key, public_key, receiver_base_transaction, transaction_description, transaction_hash,
    ...
    base_transactions = []
    for input_base_transaction in input_base_transactions:
        base_transactions.append(input_base_transaction)
    base_transactions.append(full_node_fee)
    base_transactions.append(network_fee_data)
    base_transactions.append(receiver_base_transaction)
